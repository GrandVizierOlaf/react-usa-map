...
class USAMap extends React.Component {
  ...
  addStateClass = (state) => {
    if (this.props.customize && this.props.customize[state] && this.props.customize[state].className) {
      return this.props.customize[state].className;
    }
  ...
  buildPaths = () => {
    ...
    for (let stateKey in data) {
      const path = <USAState key={stateKey} dimensions={data[stateKey]["dimensions"]} state={stateKey} fill={this.fillStateColor(stateKey)} className={this.addStateClass(stateKey)} onClickState={this.stateClickHandler(stateKey)} />;
      paths.push(path);
    };
    return paths;
  };
